...
- **可变宽度**（根据设备有不同显示策略）的图像：基于 viewport 选择
  ...
  <img srcset="pic-480.jpg 480w, 
   ...
   sizes="(max-width: 400px) 100vw, 
          ...
          640px" 
   src="pic-640.jpg" alt="pic" />
  ```

- - `w` 描述符告诉浏览器列表中的每个图象的宽度
  - 如果 `srcset` 中任何图像使用了w描述符，那么必须要设置sizes属性
...
- 方式1：设备型号选 `Responsive`，在同一任务栏最右侧的菜单中选择 `add device pixel ratio`，在出现的菜单中选择 DPR 1、2、3
- 方式2：设备型号选 `Edit` => `Add custom device`，并在 `Device pixel ratio` 中填入需要的值
